...
/* Helper Functions */
string init_user(string& filename, Server_t& server){      //initial users of server
            ...
            if(!post.is_open()){
                ...
                while (getline(post, temp) && temp[0] == '#') {
                    ...
                    if (count == 0) {
                    	...
                    	add_tag(server.tags, count, server.user[i].posts[j].tags[tag_num - 1], 5);
                    }
                ...
                for (k = 0; k < tag_num; k++) {
                    add_tag(server.tags, count, server.user[i].posts[j].tags[k], 3 * commentnum + likenum);
                }
                tag_num = 0;               
                post.close();
            }
...
User_t *get_user(Server_t& server, const string& name){            //return a pointer which point to the user I need
    unsigned int i;
    for(i = 0; i < server.user_num; i++){
...
void find_user(ifstream& file, Server_t& server, User_t *users[], unsigned int number){                  //get pointers of num to the object such as user[i].followers
    string temp;
    unsigned int i;
    for(i = 0; i < number; i++){
        getline(file, temp);
...
int catch_user(User_t *users, unsigned int num, string& name){      //return the position number of a user in server.user[] or return -1 if not find user
    unsigned int i;
    for(i = 0; i < num; i++){
...
int catch_tag(Tag_t *tags, unsigned int num, string& name){          //return the position number of a tag in server.tags[] or return -1 if not find the tag
...
void remove_user(User_t *users[], int num, User_t *target){       //to remove a user from an array, just used for unfollow as the following array in User_t is **
    int i, n;
...
void comment(Server_t& server, User_t *users, Tag_t *tags, string& username1, string &username2, unsigned int num, string& text){
    int usernum1 = catch_user(users, server.user_num, username1);
    int usernum2 = catch_user(users, server.user_num, username2);
    unsigned int i;
    int temp;
...
void uncomment(Server_t& server, User_t *users, Tag_t *tags, string& username1, string& username2, unsigned int postnum, unsigned int commentnum){
    int usernum2 = catch_user(users, server.user_num, username2);
    unsigned int i;
    int temp1;
    unsigned int temp2;
...
void delete_post(Server_t& server, User_t *users, Tag_t *tags, unsigned int num, string& username){
    int usernum = catch_user(users, server.user_num, username);
    unsigned int i;
    int temp1;
    unsigned temp2;
...
void refresh(Server_t& server, User_t *users, string& username){
    ...
    for(i = 0; i < users[usernum].num_following; i++){
        for(j = 1; j <= users[usernum].following[i]->num_posts; j++){
...
void trending(Server_t& server, Tag_t *tags, unsigned int num){
    unsigned int i, j;
    Tag_t temp;
    string text1, text2;
    for(i = 1; i < server.tag_num; i++){
        for(j = 0; j < server.tag_num - i; j++){
            if(server.tags[j].tag_score < server.tags[j + 1].tag_score){
                temp = server.tags[j + 1];
                server.tags[j + 1] = server.tags[j];
                server.tags[j] = temp;
            }
            else if(server.tags[j].tag_score == server.tags[j + 1].tag_score){
                text1 = server.tags[j].tag_content;
                text2 = server.tags[j + 1].tag_content;
                if(text1 > text2){
                    temp = server.tags[j + 1];
                    server.tags[j + 1] = server.tags[j];
                    server.tags[j] = temp;
