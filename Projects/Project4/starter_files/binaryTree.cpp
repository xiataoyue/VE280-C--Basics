...
static Node *new_root(Node *node){                  //used for copy
    if(node == NULL) return NULL;
    Node *temp = new Node(node->getstr(), node->getnum(), new_root(node->leftSubtree()), new_root(node->rightSubtree()));
    return temp;
}

static Node *find_node(Node *node, Node *tree){        //to find the root node which has the same str as given, used in contained_by function
    if(node == NULL) return NULL;
